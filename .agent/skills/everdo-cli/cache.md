...
If you suspect cache drift (items missing, edits not showing), `refresh --force` fixes it. If that still looks wrong, delete the state file and run any command — a cold start re-pulls the full database. The state file lives at `$XDG_CONFIG_HOME/everdo/state.json` (or `~/.config/everdo/state.json` by default); the portable per-checkout fallback is `<repo-root>/.config/everdo/state.json`.
